...
AABStageGimmick::AABStageGimmick()
{
	...
	// Gate Section
	static FName GateSockets[] = { TEXT("+XGate"), TEXT("-XGate"), TEXT("+YGate"), TEXT("-YGate") }; // 게이트소켓4개지정
	static ConstructorHelpers::FObjectFinder<UStaticMesh> GateMeshRef(TEXT("/Script/Engine.StaticMesh'/Game/ArenaBattle/Environment/Props/SM_GATE.SM_GATE'"));
	for (FName GateSocket : GateSockets) // 4개의 문에 대해 트리거볼륨 지정
	{
		...
		GateTrigger->ComponentTags.Add(GateSocket); // 만들어진문마다 FName정보를 태그추가
